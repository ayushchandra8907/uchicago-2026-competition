...
def build_a_news_tracker_report(events: list[dict[str, Any]], config: StrategyConfig | None = None) -> dict[str, Any]:
    live_config = config or StrategyConfig()
...
def _analyze_a_news_event(
    ...
    *,
    ...
    headline = str(news_event.get("content") or "")
    ...
    first_trade_decision = _first_matching_event(
        ...
        lambda event: (
            ...
            and str(event.get("desired_intent") or "") == "post_news_shock_take"
            and event.get("desired_side") in {"BUY", "SELL"}
        ),
    )
    first_fill = _first_matching_event(
        ...
        lambda event: (
            event_ms <= int(event.get("monotonic_ms", 0)) <= window_end_ms
            and str(event.get("intent") or "") == "post_news_shock_take"
            and event.get("side") in {"BUY", "SELL"}
        ),
    )

    first_desired_side = None if first_trade_decision is None else str(first_trade_decision.get("desired_side"))
    first_desired_qty = None if first_trade_decision is None or first_trade_decision.get("desired_qty") is None else int(first_trade_decision.get("desired_qty"))
    ...
    if first_trade_decision is not None and first_trade_decision.get("target_inventory") is not None:
        target_inventory = int(first_trade_decision.get("target_inventory"))
...
def _ideal_inventory_from_move(*, delta_ticks: float, score: float, bucket: str, config: StrategyConfig) -> int:
    direction = _sign(delta_ticks)
    if direction == 0:
        return 0
    edge_abs = abs(delta_ticks)
    min_edge = max(1, config.shock_min_edge_ticks)
    if edge_abs < min_edge:
        return 0
    scaled_cap = max(0, min(config.position_cap, config.max_absolute_position))
    confidence_span = max(1, config.shock_full_confidence_edge_ticks - min_edge)
    confidence = min(1.0, max(0.0, edge_abs - min_edge) / confidence_span)
    base_target = max(config.shock_min_position, round(scaled_cap * confidence))
    scaled_target = max(base_target, round(edge_abs * config.shock_position_scale))
    target_abs = min(scaled_cap, scaled_target)

    change_span = max(1, config.shock_full_confidence_change_ticks - min_edge)
    change_confidence = min(1.0, max(0.0, edge_abs - min_edge) / change_span)
    change_base_target = max(config.shock_min_position, round(scaled_cap * change_confidence))
    change_scaled_target = max(change_base_target, round(edge_abs * config.shock_change_position_scale))
...
def _news_position_cap(config: StrategyConfig, *, score: float, bucket: str) -> int:
    absolute = abs(score)
    if absolute >= 5.0:
